...
class LittleSistersVocabTest(unittest.TestCase):
    ...
    def test_adjective_to_verb(self):
        ...
        for variant, sentence, index, result in zip(
            number_of_variants, input_data, index_data, result_data
        ):
            with self.subTest(
                f"variation #{variant}", sentence=sentence, index=index, result=result
            ):
                self.assertEqual(
